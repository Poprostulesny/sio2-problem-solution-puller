...
def extract_link_structure(pages, browser: webdriver.Edge):
    if pages == 1:
        return link_structure([], browser), 0
    else:
        tasks_links_tab = []
        base_url = get_base_url(browser.current_url) + "/p"
        for i in range(pages):
            browser.get(base_url + "/?page=" + str(i + 1))
            tasks_links_tab, erl = link_structure(tasks_links_tab, browser)
    return tasks_links_tab, erl
...
def link_structure(tasks_links_old, browser: webdriver.Edge):
    ...
    for i in range(len(links)):
        if t < len(tasks_subjects):
            if links[i].location["y"] < tasks_subjects[t].location["y"]:
                ...
                tasks_links.append(
                    [make_name_suitable(tasks_subjects[t - 1].text), pom]
                )
                pom = []
                t += 1
                pom.append(
    ...
    tasks_links.append(
        [make_name_suitable(tasks_subjects[len(tasks_subjects) - 1].text), pom]
    )

    if len(tasks_links_old) == 0:
        return tasks_links, 0
...
def extract_result_structure(pages, browser: webdriver.Edge):
    ...
    for i in range(pages):
        browser.get(base_url + "/?page=" + str(i + 1))
        results, err = result_structure(browser, results)
    return results, err
...
def result_structure(browser: webdriver.Edge, result_structure_old):
    ...
    try:
        ...
        czas = browser.find_element(
            By.XPATH, "//*[contains(text(), 'Czas zgłoszenia')]"
        )
        margines = browser.find_element(By.CLASS_NAME, "submission__margin")
        score = browser.find_elements(
            locate_with(By.TAG_NAME, "td").below(wynik).to_right_of(zgloszenia)
        )
        tasks = browser.find_elements(
    ...
    for i in range(len(tasks)):
        if score[i].text == "":
            result_structure_old.append(
                {
                    "id": id_from_name(tasks[i].text),
                    "score": -1,
            ...
            result_structure_old.append(
                {
                    "id": id_from_name(tasks[i].text),
                    "score": int(score[i].text),
                    "link": links[i].get_attribute("href"),
                }
            )

    return result_structure_old, 0
...
def file_download(
    path_temp, path_end, url, browser: webdriver.Edge, task, name, dir_down, files
):
    if task == 1:
            ...
            if "application/pdf" in embed.get_attribute("type"):
                ...
                if not os.path.exists(dir_down + "\\" + namep):
                    ...
                    if not os.path.exists(path_end + "\\" + namep):
                        os.rename(
                            dir_down + "\\" + namep, path_end + "\\" + name + ".pdf"
                        )
                    else:
    ...
    for i in tab:
        ...
        for y in i[1]:
            ...
            if y["solved"] == 1:
                ...
                file_download(
                    dir_temp, dir_new_p, y["href"], browser, 1, y["text"], dir_down, f
                )
